    ...
    <div class="wire-form">
        ...
        <div class="col-12">
            ...
            <div class="row invoice-info">
                <div class="col-lg-12">
                    <form action="{{ route('admin.penjualan.update') }}" onsubmit="return kirim()" name="form" id="form"
                                    ...
                                    <div class="col-md-12">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-8">
                                                ...
                                                <div class="col-sm-12">
                                                    <button type="submit" name="_i" value="{{$no_penjualan}}" class="btn btn-primary btn-sm">Simpan</button>
                                                    <a href="">Kembali</a>
                                                </div>
...
@push('js')
    <script>
        $(document).ready(function() {
            ...
            // Loop through the penjualanDetails and populate incTbl
            @foreach ($produk_jual as $detail)
                $('.incTbl').append(`
                    <tr>
                        <td>
                            <input type="hidden" readonly class="kodein" name="produk_id[]" value="{{ $detail->produk_id }}">
                            {{ $detail->produk->nama_produk }} ({{ $detail->produk_id }})
                        </td>
                        <td>
                            <input type="hidden" readonly class="kodein" name="jumlah[]" value="{{ $detail->jumlah }}">
                            {{ $detail->jumlah }}
                        </td>
                        <td>
                            <input type="hidden" readonly class="kodein" name="harga[]" value="{{ $detail->produk->harga_jual }}">
                            {{ $detail->produk->harga_jual }}
                        </td>
                        <td>
                            <input type="hidden" readonly class="kodein" name="total[]" value="{{$detail->produk->harga_jual* $detail->jumlah }}">
                            {{$detail->produk->harga_jual* $detail->jumlah  }}
                        </td>
                        <td>
                            <span class="btn btn-sm btn-warning btn-delete">
